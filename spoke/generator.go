...
func GenerateDrupalSpoke(name, bundle, configPath string) (*ProtoFile, error) {
	...
	// Generate proto
	proto := &ProtoFile{
		Package:     fmt.Sprintf("spoke.%s.v1", name),
		GoPackage:   fmt.Sprintf("github.com/lehigh-university-libraries/crosswalk/spoke/%s/v1;%sv1", name, name),
		PackageName: fmt.Sprintf("%sv1", name),
